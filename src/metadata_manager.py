...
def load_metadata(rom_path):
    ...
    if metadata_path and os.path.exists(metadata_path):
        try:
            ...
            logging.error(f"Errore nel caricamento dei metadati da {metadata_path}: {e}")
            return None
    return None
...
def delete_metadata_and_cover(rom_path):
    ...
    if metadata_path:
        ...
        for ext in possible_exts:
            potential_cover_path = os.path.join(COVERS_FOLDER, os.path.basename(metadata_base) + ext)
        ...
        if not found_cover:
             logging.warning(f"Nessun file copertina trovato per {rom_path} in {COVERS_FOLDER}")
    ...
    try:
        if metadata_path and os.path.exists(metadata_path):
            ...
            logging.warning(f"File metadati non trovato o già eliminato: {metadata_path}")
    except Exception as e:
        err_msg = f"Errore eliminazione metadati ({os.path.basename(metadata_path)}): {e}"
        errors.append(err_msg)
        logging.error(err_msg)
