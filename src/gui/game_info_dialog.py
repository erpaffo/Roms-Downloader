        ...
        if filepath:
            if not self.metadata_path:
                QMessageBox.critical(
                    self,
                    "Errore Interno",
                    "Impossibile determinare il nome per la copertina.",
                )
                return
            ...
            try:
                ...
                if (
                    old_cover
                    and os.path.exists(old_cover)
                    and old_cover != new_cover_path
                    ...
                    try:
                        ...
                        logging.warning(
                            f"Impossibile rimuovere vecchia copertina {old_cover}: {remove_err}"
                        )
                ...
                self._load_cover()
                logging.info(
                    f"Nuova copertina selezionata e copiata in: {new_cover_path}"
                )

            except Exception as e:
                logging.error(
                    f"Errore nel copiare/gestire la nuova copertina da {filepath} a {new_cover_path}: {e}"
                )
                QMessageBox.warning(
                    self,
                    "Errore Copertina",
                    f"Impossibile impostare la nuova immagine:\n{e}",
                )
    ...
    def save_changes(self):
        if not self.metadata_path:
            QMessageBox.critical(
                self,
                "Errore Interno",
                "Percorso metadati non valido, impossibile salvare.",
            )
            return
        ...
        self.game_data["genres"] = [
            g.strip() for g in self.genres_edit.text().split(",") if g.strip()
        ]
        self.game_data["languages"] = [
            l.strip() for l in self.langs_edit.text().split(",") if l.strip()
        ]
        self.game_data["user_edited"] = True
        ...
        try:
            if save_metadata(self.game_data["rom_path"], self.game_data):
                logging.info(
                    f"Metadati salvati per {self.game_data['rom_path']} in {self.metadata_path}"
                )
                self.accept()
            else:
                QMessageBox.critical(
                    self,
                    "Errore Salvataggio",
                    f"Impossibile salvare i metadati nel file:\n{self.metadata_path}",
                )

        except Exception as e:
            logging.exception(
                f"Errore imprevisto nel salvataggio dei metadati in {self.metadata_path}: {e}"
            )
            QMessageBox.critical(
                self,
                "Errore Salvataggio Imprevisto",
    ...
    def delete_game(self):
        ...
        if not rom_path or not self.metadata_path:
            QMessageBox.critical(
                self,
                "Errore Interno",
                "Informazioni sul percorso del gioco o dei metadati mancanti.",
            )
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            try:
                if os.path.exists(rom_path):
                    ...
                    logging.warning(
                        f"  - File ROM non trovato o già eliminato: {rom_path}"
                    )
            except Exception as e:
                logging.error(f"  - Errore eliminazione ROM {rom_path}: {e}")
                QMessageBox.warning(
                    self,
                    "Errore Eliminazione ROM",
                    f"Impossibile eliminare il file ROM:\n{e}",
                )
            ...
            if meta_errors:
                QMessageBox.warning(
                    self,
                    "Eliminazione Metadati/Copertina con Errori",
