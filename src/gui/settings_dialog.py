...
from PySide6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel,
                               ...
                               QMessageBox, QGroupBox, QFormLayout, QComboBox, QApplication) # Aggiunto QComboBox, QApplication
from PySide6.QtCore import Qt, QSettings # Aggiunto QSettings
from src.config import (STYLES_REL_PATH, USER_DOWNLOADS_FOLDER, set_user_download_folder,
                        MAX_CONCURRENT_DOWNLOADS, set_max_concurrent_downloads,
                        CONSOLES, add_console, resource_path, # Aggiunto resource_path
                        SETTINGS_ORG, SETTINGS_APP, DEFAULT_THEME_FILENAME)
...
class SettingsDialog(QDialog):
        ...
        try:
            ...
            for filename in os.listdir(styles_dir):
                if filename.lower().endswith("_theme.qss"):
                    theme_name_base = filename[:-len("_theme.qss")] # Es: "light", "dark_blue"
                    display_name = theme_name_base.replace('_', ' ').title() + " Theme" # Es: "Light Theme", "Dark Blue Theme"
            ...
            if DEFAULT_THEME_FILENAME not in self.theme_map.values():
                 if os.path.exists(os.path.join(styles_dir, DEFAULT_THEME_FILENAME)):
                      self.theme_combo.insertItem(0, default_display_name) # Inserisci all'inizio
                      self.theme_map[default_display_name] = DEFAULT_THEME_FILENAME
                 else:
                     # Se anche il default non esiste, scegli il primo trovato come fallback
                     if self.theme_map:
                         first_display_name = list(self.theme_map.keys())[0]
                         self.theme_map[default_display_name] = self.theme_map[first_display_name]
                         logging.warning(f"File tema di default '{DEFAULT_THEME_FILENAME}' non trovato. Usando '{self.theme_map[default_display_name]}' come fallback.")
                     else:
            ...
            if not found_themes and default_display_name not in self.theme_map:
                 logging.warning(f"Nessun file *_theme.qss trovato in {styles_dir} e il default non è disponibile.")
    ...
    def _load_current_settings(self):
        ...
        current_dl_folder = self.settings.value("download_folder", USER_DOWNLOADS_FOLDER)
        ...
        current_theme_filename = self.settings.value("gui/theme", DEFAULT_THEME_FILENAME)
        ...
        if selected_display_name:
            ...
            default_display = "Default" if DEFAULT_THEME_FILENAME in self.theme_map.values() else None
            ...
            logging.warning(f"Tema salvato ('{current_theme_filename}') non trovato. Selezionato tema di default/primo disponibile.")


    def browse_folder(self):
        """Opens a dialog to select the download folder."""
        current_folder = self.download_folder_edit.text()
        folder = QFileDialog.getExistingDirectory(self, "Seleziona cartella download", current_folder)
        if folder:
    ...
    def add_new_console(self):
        ...
        try:
            ...
            logging.info(f"Nuova console aggiunta/aggiornata (runtime): {name} -> {link}")
    ...
    def accept_settings(self):
        ...
        if new_folder != current_folder:
             set_user_download_folder(new_folder) # Questa funzione salva già in settings
        ...
        if new_max_dl != current_max_dl:
             set_max_concurrent_downloads(new_max_dl) # Questa funzione salva già in settings
        ...
        if selected_filename is not None and selected_filename != current_theme:
            ...
            self.apply_theme_immediately(selected_filename) # Chiamata per applicazione immediata
    ...
    def apply_theme_immediately(self, theme_filename):
        ...
        if os.path.exists(theme_path):
            try:
                ...
                logging.error(f"Errore lettura tema per applicazione immediata {theme_path}: {e}")
        else:
             logging.warning(f"File tema '{theme_filename}' non trovato per applicazione immediata.")

        app = QApplication.instance()
        if app:
             app.setStyleSheet(style_sheet)
        else:
             logging.error("Impossibile ottenere QApplication instance per applicare il tema.")
    ...
    def get_values(self):
        ...
        """
        selected_display_name = self.theme_combo.currentText()
        selected_theme_filename = self.theme_map.get(selected_display_name, DEFAULT_THEME_FILENAME)
