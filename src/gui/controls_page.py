...
PRETTY_LABELS = {
    ...
    "input_player1_l2": "L2",
    "input_player1_r2": "R2",
    "input_player1_l3": "L3 (Stick Sinistro)",
    "input_player1_r3": "R3 (Stick Destro)",
    "input_player1_z": "Z",
    "input_player1_c_up": "C-Su",
    "input_player1_c_down": "C-Giù",
    "input_player1_c_left": "C-Sinistra",
    "input_player1_c_right": "C-Destra",
    "input_player1_cross": "Cross (X)",
    "input_player1_circle": "Circle (O)",
    "input_player1_square": "Square",
    "input_player1_triangle": "Triangle",
    "input_player1_analog_left": "Stick Sinistro - Sinistra",
    "input_player1_analog_right": "Stick Sinistro - Destra",
    "input_player1_analog_up": "Stick Sinistro - Su",
    "input_player1_analog_down": "Stick Sinistro - Giù",
    "input_player1_analog_r_left": "Stick Destro - Sinistra",
    "input_player1_analog_r_right": "Stick Destro - Destra",
    "input_player1_analog_r_up": "Stick Destro - Su",
    "input_player1_analog_r_down": "Stick Destro - Giù",
    "input_player1_dpad_up": "D-Pad Su",
    "input_player1_dpad_down": "D-Pad Giù",
    "input_player1_dpad_left": "D-Pad Sinistra",
    "input_player1_dpad_right": "D-Pad Destra",
    "input_player1_fire": "Fuoco 1",
    "input_player1_fire2": "Fuoco 2",
    "input_player1_pause": "Pausa",
    "input_player1_ps": "Tasto PS/Home",
    "input_player1_zl": "ZL",
    "input_player1_zr": "ZR",
    "input_player1_minus": "Minus (-)",
    "input_player1_plus": "Plus (+)",
    "input_player1_home": "Home",
    "input_player1_1": "1",
    "input_player1_2": "2",
    "input_player1_c": "C (Nunchuk)",
    "input_menu_toggle": "Menu Principale",
    "input_save_state": "Salva Stato",
    "input_load_state": "Carica Stato",
    "input_exit_emulator": "Esci Emulatore",
    "input_pause_toggle": "Pausa/Riprendi",
    "input_reset": "Reset Gioco",
    "input_rewind": "Rewind",
    "input_hold_fast_forward": "Avanzamento Rapido (Tieni)",
    "input_toggle_fast_forward": "Avanzamento Rapido (Toggle)",
    "input_screenshot": "Screenshot",
    "input_toggle_fullscreen": "Schermo Intero",
    "input_volume_up": "Volume Su",
    "input_volume_down": "Volume Giù",
    "input_shader_next": "Shader Successivo",
    "input_shader_prev": "Shader Precedente",
    "input_shader_toggle": "Attiva/Disattiva Shader",
    "input_state_slot_increase": "Slot Stato (+)",
    "input_state_slot_decrease": "Slot Stato (-)",
    "video_vsync": "V-Sync",
    "video_driver": "Driver Video",
    "fps_show": "Mostra FPS",
    "input_cheat_index_minus": "Cheat Index (-)",
    "input_cheat_index_plus": "Cheat Index (+)",
    "input_cheat_toggle": "Attiva/Disattiva Cheat",
    "input_desktop_menu_toggle": "Menu Desktop",
    "input_driver": "Driver Input",
    "input_grab_mouse_toggle": "Cattura/Rilascia Mouse",
    "input_hold_slowmotion": "Slow Motion (Tieni)",
    "input_netplay_game_watch": "Netplay Guarda",
    "audio_volume": "Volume Audio (dB)",
    "video_scale": "Scala Video",
    "audio_driver": "Driver Audio",
    "video_aspect_ratio": "Aspect Ratio",
    "video_fullscreen": "Schermo Intero (Opzione)",
    "menu_font_color_red": "Colore Font Menu (Rosso)",
    "menu_font_color_green": "Colore Font Menu (Verde)",
    "menu_font_color_blue": "Colore Font Menu (Blu)",
}
...
class ControlsPage(QWidget):
    ...
    def __init__(self, config_folder, parent=None):
        ...
        if not os.path.exists(self.config_folder):
            try:
                ...
                logging.error(
                    f"Cannot create config folder '{self.config_folder}': {e}"
                )
        ...
        self.global_config_path = os.path.join(
            self.config_folder, "retroarch_global.cfg"
        )
    ...
    def _get_scroll_content_widget(self, tab_index):
        ...
        try:
            ...
            logging.error(
                f"Error getting scroll content widget for tab {tab_index}: {e}"
            )
        return None
    ...
    def _load_config_file(self, file_path):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Errore Caricamento",
                f"Impossibile caricare {os.path.basename(file_path)}:\n{e}",
            )
        return settings_dict
    ...
    def load_bindings_and_settings(self, console_name):
        ...
        if not self.current_core_base:
            QMessageBox.critical(
                self, "Errore", f"Core base not defined for {console_name} in config.py"
            )
    ...
    def _clear_all_tabs(self):
        ...
        try:
            self._clear_layout(
                self._get_scroll_content_widget(0).layout()
                if self._get_scroll_content_widget(0)
                else None
            )
            self._clear_layout(
                self._get_scroll_content_widget(1).layout()
                if self._get_scroll_content_widget(1)
                else None
            )
            self._clear_layout(
                self._get_scroll_content_widget(2).layout()
                if self._get_scroll_content_widget(2)
                else None
            )
    ...
    def _populate_p1_controls_tab(self):
        ...
        for command in sorted_commands:
            ...
            if current_value_from_config is not None:
                ...
                display_value = (
                    convert_binding(raw_default_value)
                    if raw_default_value != "nul"
                    else "nul"
                )

            label_text = PRETTY_LABELS.get(
                command, command.replace("input_player1_", "").replace("_", " ").title()
            )
    ...
    def _populate_global_hotkeys_tab(self):
        ...
        for command in sorted_commands:
            ...
            label_text = PRETTY_LABELS.get(
                command, command.replace("input_", "").replace("_", " ").title()
            )

            is_boolean = raw_default_value.lower() in ["true", "false"] or (
                current_value_from_config is not None
                and current_value_from_config.lower() in ["true", "false"]
            )

            if is_boolean:
                current_value = (
                    current_value_from_config
                    if current_value_from_config is not None
                    else raw_default_value
                )
    ...
    def _populate_core_settings_tab(self):
        ...
        if self.current_console:
            try:
                ...
                logging.error(
                    "Funzioni get_save_directory o SYSTEM_FOLDER non trovate in config.py"
                )
        ...
        for key in path_keys_to_add:
            ...
            line_edit.setPlaceholderText(
                f"Default RetroArch o {default_paths.get(key, 'N/D')}"
            )
            browse_button = QPushButton("Sfoglia...")
            browse_button.clicked.connect(
                lambda checked=False, le=line_edit, start=default_paths.get(
                    key, os.path.expanduser("~")
                ): self._browse_directory(le, start)
            )
        ...
        current_index = vdriver_widget.findText(
            vdriver_current_value,
        ...
        if current_index >= 0:
            ...
            logging.warning(
                f"Saved value '{vdriver_current_value}' for {vdriver_key} not found in standard list. Selecting default '{vdriver_default}'."
            )
            default_index = vdriver_widget.findText(vdriver_default)
            if default_index >= 0:
                vdriver_widget.setCurrentIndex(default_index)
        ...
        core_options_to_show = sorted(
            list(
                set(CORE_USEFUL_OPTIONS + CORE_COMMON_ALLOCATED_OPTIONS)
                - options_to_exclude
            )
        )

        for key in core_options_to_show:
            current_value = self.core_settings.get(
                key, CORE_SETTINGS_DEFAULTS.get(key, "")
            )
            is_boolean = current_value.lower() in [
                "true",
                "false",
            ] or CORE_SETTINGS_DEFAULTS.get(key, "").lower() in ["true", "false"]
            label_text = PRETTY_LABELS.get(key, key.replace("_", " ").title()) + ":"
            if is_boolean:
                input_widget = QCheckBox()
                input_widget.setChecked(current_value.lower() == "true")
        ...
        if current_index == 0:
            ...
            for command, widget in self.global_hotkey_widgets.items():
                ...
                if isinstance(widget, QCheckBox):
                    ...
                    widget.setText(
                        converted_default if converted_default != "nul" else ""
                    )
    ...
    def on_save(self):
        ...
        if not self.current_console or not self.current_core_base:
            QMessageBox.warning(
                self, "Errore", "Nessuna console/core valido selezionato."
            )
            return
        ...
        for command, value in combined_settings.items():
            if (
                command in all_hotkey_widgets
                and isinstance(all_hotkey_widgets[command], HotkeyInput)
                and value != "nul"
            ):
        ...
        if self.core_config_path:
            try:
                ...
                logging.info(
                    f"Impostazioni COMBINATE (con percorsi UI) per '{self.current_console}' salvate in: {self.core_config_path}"
                )
            except Exception as e:
                success = False
                errors.append(
                    f"Errore salvataggio file core ({os.path.basename(self.core_config_path)}): {e}"
                )
    ...
    def _browse_directory(self, target_line_edit: QLineEdit, start_dir: str = ""):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Seleziona Cartella", initial_dir
        )
        if folder:
