...
class GameApiClient:
    def __init__(self):
        ...
        if not self.twitch_client_id or not self.twitch_client_secret:
            logging.warning("Credenziali Twitch (Client ID/Secret) non trovate nelle variabili d'ambiente. Le ricerche IGDB falliranno.")
        if not self.rawg_api_key:
            logging.warning("API Key RAWG non trovata nelle variabili d'ambiente. Le ricerche RAWG falliranno.")
    ...
    def _get_twitch_token(self):
        if not self.twitch_client_id or not self.twitch_client_secret:
            logging.error("Impossibile ottenere token Twitch: Client ID o Secret mancanti.")
            return False
        ...
        try:
            response = requests.post(self.twitch_token_url, data=payload, headers=headers, timeout=10)
            response.raise_for_status()
            ...
            logging.info(f"Nuovo token Twitch ottenuto, valido per circa {expires_in // 3600} ore.")
            return True
            ...
            return False
             ...
             logging.error(f"Errore decodifica risposta JSON token Twitch: {response.text}")
             self.twitch_token = None
             return False
    ...
    def _make_igdb_request(self, endpoint, query_body):
        ...
        try:
            response = requests.post(full_url, headers=headers, data=query_body.encode('utf-8'), timeout=15)
            response.raise_for_status()
            ...
            return None
             ...
             logging.error(f"Errore decodifica risposta JSON IGDB da '{endpoint}': {response.text}")
             return None

    def search_igdb(self, title, platform_name=None):
        if not self.twitch_client_id: return None
        ...
        if platform_id:
            ...
            logging.warning(f"[IGDB] ID piattaforma non trovato per '{platform_name}', ricerca senza filtro piattaforma.")
        ...
        release_date_str = datetime.fromtimestamp(release_ts).strftime('%Y-%m-%d') if release_ts else None
    ...
    def search_rawg(self, title, platform_name=None):
        if not self.rawg_api_key: return None
        ...
        if platform_id:
            ...
            logging.warning(f"[RAWG] ID piattaforma non trovato per '{platform_name}', ricerca senza filtro piattaforma.")
        ...
        try:
            ...
            if data and data.get('results'):
                game = data['results'][0]
                logging.info(f"[RAWG] Trovato risultato per '{title}': {game.get('name')}")
            ...
            return None
             ...
             logging.error(f"Errore decodifica risposta JSON RAWG per '{title}': {response.text}")
             return None

game_api_client_instance = GameApiClient()

def fetch_game_details(original_filename, console_name):
    global game_api_client_instance
    ...
    logging.info(f"[API Fetch] Inizio ricerca per '{original_filename}' -> Pulito: '{cleaned_title}'")
    ...
    if simplified != cleaned_title and simplified.lower() not in [t.lower() for t in search_attempts]:
        search_attempts.append(simplified)
    if ' - ' in cleaned_title:
        base_title = cleaned_title.split(' - ')[0].strip()
        if base_title != cleaned_title and base_title.lower() not in [t.lower() for t in search_attempts]:
    ...
    for attempt_title in unique_attempts:
        logging.debug(f"[API Fetch] Tentativo IGDB con '{attempt_title}' (Console: {console_name})...")
        igdb_details = game_api_client_instance.search_igdb(attempt_title, console_name)
        if igdb_details:
            api_title = igdb_details.get('api_title', '')
            similarity_score = fuzz.token_set_ratio(attempt_title.lower(), api_title.lower())
            logging.debug(f"[API Validation IGDB] Confronto: '{attempt_title}' vs API '{api_title}' -> Score: {similarity_score}")
            if similarity_score >= MIN_SIMILARITY_THRESHOLD:
                logging.info(f"[API Fetch] SUCCESSO (IGDB): Trovato '{api_title}' con score {similarity_score} >= {MIN_SIMILARITY_THRESHOLD}")
                return igdb_details
            else:
                logging.warning(f"[API Fetch] RISULTATO SCARTATO (IGDB): API ha trovato '{api_title}', ma similarità ({similarity_score}) troppo bassa rispetto a '{attempt_title}'.")

        logging.debug(f"[API Fetch] Tentativo RAWG con '{attempt_title}' (Console: {console_name})...")
        rawg_details = game_api_client_instance.search_rawg(attempt_title, console_name)
        if rawg_details:
            api_title = rawg_details.get('api_title', '')
            similarity_score = fuzz.token_set_ratio(attempt_title.lower(), api_title.lower())
            logging.debug(f"[API Validation RAWG] Confronto: '{attempt_title}' vs API '{api_title}' -> Score: {similarity_score}")
            if similarity_score >= MIN_SIMILARITY_THRESHOLD:
                logging.info(f"[API Fetch] SUCCESSO (RAWG): Trovato '{api_title}' con score {similarity_score} >= {MIN_SIMILARITY_THRESHOLD}")
                return rawg_details
            else:
                logging.warning(f"[API Fetch] RISULTATO SCARTATO (RAWG): API ha trovato '{api_title}', ma similarità ({similarity_score}) troppo bassa rispetto a '{attempt_title}'.")

    logging.warning(f"[API Fetch] FALLIMENTO TOTALE: Nessun risultato valido trovato per '{original_filename}' dopo tutti i tentativi e validazioni.")
    return None
