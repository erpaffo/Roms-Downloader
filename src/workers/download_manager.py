...
class DownloadManager(QObject):
    ...
    finished = Signal()
    file_progress = Signal(str, int, int, float, float)  # (game_name, percent_file)
    ...
    def on_worker_progress(self, game_name, downloaded, total, speed, remaining_time):
        ...
        # Ora invia correttamente sempre 5 parametri:
    ...
    def on_worker_finished(self, game_name, local_filename, worker, thread):
        ...
        if local_filename:
            ...
            # Invia i dati finali completi, con velocità e tempo residuo pari a 0:
            self.file_progress.emit(game_name, 100, 100, 0.0, 0.0)

        self.file_finished.emit(game_name)

        # Rimuove il worker dalla lista degli attivi
